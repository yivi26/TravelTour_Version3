...
function renderProgressPanelContent(tour, days, completedSet, guideCompletedAt = null) {
  ...
  const disableChecks = tourCompleted ? " disabled" : "";
          ...
          const activitiesHtml = day.activities.length
            ? day.activities
                .map((act) => {
                  ...
                  return `
              <label class="tour-progress__activity${checked ? " is-done" : ""}">
                <input
                  type="checkbox"
                  data-progress-check
                  data-tour-id="${tour.id}"
                  data-activity-id="${escapeHtml(act.id)}"
                  ${checked ? "checked" : ""}${disableChecks}
                />
...
function bindEvents() {
  ...
  document.addEventListener("change", async function (event) {
    ...
    let completedSet = getProgressSetForTour(tourId);
    try {
      if (wasChecked) completedSet.add(activityId);
      else completedSet.delete(activityId);
